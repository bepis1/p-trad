...
(function() {
    ...
    // Module-level constants — declared here (top of the IIFE) so that the
    // main-execution section (04) can safely call tracker functions whose
    // bodies reference these. A `const` inside section 16 would still be in the
    // temporal dead zone when section 04 runs, because section 04's top-level
    // statements execute before section 16's text is reached.
    const TRACKER_KEY = 'trade_tracker_v1';

    if (GM_getValue('current_logistics_route', null)) {
        GM_deleteValue('current_logistics_route');
    }
    ...
    // --- 2. Ground-Truth Cargo Scanner w/ Phantom Protection ---
    function syncCargoFromNav() {
        ...
        if (freeMatch) {
            ...
            if (freeMatch[2] !== undefined) {
                magScoopFree = parseInt(freeMatch[2], 10);
                hasMagScoop = true;
            }

            let magScoopSize = magScoopUsed + magScoopFree;
            GM_setValue('logistics_mag_scoop_size', hasMagScoop ? magScoopSize : 0);
    ...
    // --- 3. True AP-Density Simulation Engine ---

    // Resolve the player's current sector from the page's unsafeWindow.userloc
    // so the sim can reuse the local-sector pathfinder's Dijkstra for real
    // terrain-aware AP instead of straight-line Manhattan. Returns null when
    // the page has no userloc or the static map isn't loaded (callers fall back
    // to Manhattan * 5, the open-space minimum).
    ...
    // Falls back to Manhattan * 5 when no sector, no map, or target unreachable.
    function simTravelAP(fromCoords, toCoords, sectorName, dijCache) {
        if (!fromCoords || !toCoords) return 0;
        if (!sectorName) return getDistance(fromCoords, toCoords) * 5;
        const key = sectorName + '|' + fromCoords.x + ',' + fromCoords.y;
        let map = dijCache[key];
        if (map === undefined) {
            try { map = getSectorAllDistances(sectorName, fromCoords.x, fromCoords.y); }
            catch (e) { map = null; }
            dijCache[key] = map;
        }
        if (map) {
        ...
        return getDistance(fromCoords, toCoords) * 5;
    }
    ...
    // before the pathfinder module has been loaded.
...
const SECTOR_DATA = {
    ...
    // --- 5. Buildings Tab: Bookkeeper Parser ---
    ...
    // --- 6. Main Nav Screen: Draggable Control Center ---
    function injectDraggableUI() {
        ...
        function fmtCrLocal(n) {
            if (n == null || isNaN(n)) return '?';
            return simpleNumberFormatTracker(n);
        }
        function fmtRatioLocal(r) {
        ...
        if (safeSteps.length === 0) {
            ...
            safeSteps.forEach((step, index) => {
                ...
                const profitTxt = isTo
                    ...
                    : (hasPrice
                        ? `<span style="color:${profitColor(e.profit)};">${e.profit >= 0 ? '+' : ''}${fmtCrLocal(e.profit)} cr</span>`
                        : '<span style="color:#666;">no price</span>');
                const ratioTxt = (hasPrice && e.ratio != null)
                    ? `<span style="color:#00ff88;">${fmtRatioLocal(e.ratio)}</span>`
                    : '<span style="color:#666;">?</span>';
                const cumProfitTxt = `<span style="color:${profitColor(e.cumProfit)};">${e.cumProfit >= 0 ? '+' : ''}${fmtCrLocal(e.cumProfit)} cr</span>`;
                const cumRatioTxt = (e.cumRatio != null)
                    ? `<span style="color:#00ddaa;">${fmtRatioLocal(e.cumRatio)}</span>`
                    : '<span style="color:#666;">?</span>';
        ...
        document.getElementById('nav-btn-clear').addEventListener('click', () => {
            ...
            GM_deleteValue('logistics_mag_scoop_size');
    ...
    // TDZ-safe: SECTOR_DATA is a const defined later in the IIFE; accessing it
    // before initialization throws ReferenceError, so we guard with try/catch.
    ...
    function getSectorAllDistances(sectorName, startX, startY) {
        ...
        // Diagnostic: count terrain types and log target tile.
        if (typeof unsafeWindow !== 'undefined') {
            ...
            console.log('[pardus-tracker] Grid', sectorName, rows + 'x' + cols,
                'terrain counts:', terrainCounts);
        }
        if (startX < 0 || startY < 0 || startX >= cols || startY >= rows) return null;
    ...
    function flyToCoords(target, destLabel, onArrive) {
        ...
        function flyNext() {
            ...
            if (idx < 0) {
                ...
                const curCoordEl = document.getElementById('coords');
                const curParsed = curCoordEl ? parseCoords(curCoordEl.innerText) : null;
                if (curParsed) {
                    let bestIdx = -1, bestDist = Infinity;
                    for (let j = 0; j < pathCoords.length; j++) {
                        const d = Math.abs(pathCoords[j].x - curParsed.x) +
                                 Math.abs(pathCoords[j].y - curParsed.y);
                        if (d < bestDist) { bestDist = d; bestIdx = j; }
                    }
                    if (bestIdx >= 0) {
                        idx = bestIdx;
                        setOverlay(`\u2708 Re-synced to nearest path tile (${bestDist} away). Continuing...`);
                    }
                }
                if (idx < 0) {
    ...
    // --- 9. Trade Screen Interceptor, Reality Sync, & UI ---
    ...
    // --- Reality Clamp: read the ACTUAL stock / free space for a commodity row
    //     straight off the trade DOM. This is the last line of defense against
    //     bookkeeper tick-projection drift caused by other players trading
    //     between the moment bookkeeper logged the building and now.
    ...
    function injectTradeHUD() {
        ...
        if (typeof w.mag_scoop !== 'undefined' && w.mag_scoop && typeof w.mag_scoop_size !== 'undefined') {
            GM_setValue('logistics_mag_scoop_size', w.mag_scoop_size);
        } else if (typeof w.mag_scoop === 'undefined' || !w.mag_scoop) {
            GM_setValue('logistics_mag_scoop_size', 0);
    ...
    // --- 10. Trade Tracker: per-location stock & price persistence ---
    //
    ...
    // NOTE: TRACKER_KEY is declared in 00-header.js (top of the IIFE) so the
    // main-execution section can call these functions before this file's text
    // is reached. Do not redeclare it here.
    ...
    function getTrackerEntry(userloc) {
        const store = getTrackerStore();
        return store[String(userloc)] || null;
    }
    ...
    // Marginal per-unit price for a single unit traded at a given stock level.
    function trackerUnitPriceAtStock(entry, resId, stockLevel, isBuying) {
        const c = entry.commodities[resId];
        if (!c) return 0;
        if (trackerUsesFlatPricing(entry)) {
            return isBuying ? c.buyFromObjPrice : c.sellToObjPrice;
        }
        if (!entry.pricing) return isBuying ? c.buyFromObjPrice : c.sellToObjPrice;
        return trackerBaseResPrice(
            entry.pricing.baseP0, entry.pricing.baseR, entry.pricing.baseBuyCharge,
            resId, c.max || 0, stockLevel, 1, isBuying
        );
    }
    ...
    // Chebyshev distance (8-directional grid, 1 AP per step minimum).
    function trackerChebyshev(c1, c2) {
        return Math.max(Math.abs(c1.x - c2.x), Math.abs(c1.y - c2.y));
    }

    // Compute AP distances from player to every tracked location.
    // Uses Dijkstra (accurate) if static map data is loaded for the sector,
    // otherwise falls back to Chebyshev × 5 (rough estimate, min terrain AP).
            ...
            if (eSector === playerSector) {
                if (dijkstraMap) {
                    ...
                    if (dijkstraMap[key] !== undefined) {
                        ...
                        // Dijkstra ran but target unreachable (blocked by b terrain).
                        // Fall back to Chebyshev estimate so user still gets a number.
                        result[k] = trackerChebyshev(playerCoords, eCoords) * 5;
                        usedFallback = true;
                    }
                } else {
                    // No map data — Chebyshev × 5 (space terrain minimum AP).
                    result[k] = trackerChebyshev(playerCoords, eCoords) * 5;
                }
            } else {
    ...
    function injectTrackerPanel() {
        ...
        function deriveDisplaySector(userloc) {
            return getSectorFromTileId(userloc);
        }
        ...
        function renderBody() {
            ...
            function buildEntryRow(e) {
                ...
                const eSector = deriveDisplaySector(e.userloc);
                ...
                const isEstimate = distInfo && distInfo.usedFallback;
                let distLabel;
                if (sortByDistance && distVal != null) {
                    const prefix = isEstimate ? '\u2248' : ''; // ≈ for estimates
                    distLabel = ' \u00b7 <span style="color:#ffaa44;">' + prefix + distVal + ' AP</span>';
        ...
        distBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                if (result.usedFallback) {
                    distBtn.textContent = 'Updated (est. — no map data)';
                } else {
            ...
            for (const k in store) {
                ...
                if (sameSector) {
                    if (toDijkstra) {
                        ...
                        if (D == null) { D = trackerChebyshev(toCoords, eCoords) * 5; estD = true; }
                    } else {
                        D = trackerChebyshev(toCoords, eCoords) * 5; estD = true;
                    }
                }
    ...
    function computeFweRoutes() {
        ...
        for (const k in store) {
            ...
            // Round-trip travel AP (hub -> starbase -> hub).
            let oneWayAp = null;
            if (hubDijkstra) {
            ...
            if (oneWayAp == null) {
                oneWayAp = trackerChebyshev(hubCoords, sbCoords) * 5;
            }
            const apCost = oneWayAp * 2 + TRADE_AP;
    ...
    function injectExportsCalculator() {
        ...
        refreshBtn.addEventListener('click', () => renderBody());
        ...
        function renderExportsBody() {
            ...
            if (res.usedFallback) {
                sumHtml += '<div style="color:#8a6a3a;margin-bottom:4px;">\u26a0 No sector map data \u2014 same-sector AP estimated (\u2248).</div>';
            }
        ...
        function renderFweBody() {
            ...
            sumHtml += '<div style="color:#aaa;margin-bottom:4px;">' +
                ...
                '</div>';
            if (res.usedFallback) {
                sumHtml += '<div style="color:#8a6a3a;margin-bottom:4px;">\u26a0 No sector map data \u2014 AP estimated (\u2248).</div>';
            }
            body.innerHTML = sumHtml;
            ...
            res.routes.forEach((r, i) => {
                ...
                tr.innerHTML =
                    ...
                    '<td style="text-align:right;color:#ffaa44;">' + r.apCost + '</td>' +
                    '<td style="text-align:right;">' + ratioTxt + '</td>';
                t.appendChild(tr);
            });
            t.addEventListener('click', function(e) {
    ...
    // --- 18. Route Economics: per-step credit/AP ratio ---
    //
    ...
    function computeRouteEconomics(steps) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            let travelAp = 0;
            try { travelAp = simTravelAP(prevLoc, loc, sector, dijCache); } catch (e) { travelAp = getDistance(prevLoc, loc) * 5; }
